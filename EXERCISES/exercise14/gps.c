...
void readCity( FILE *fp, struct City *cty ) {
    fscanf( fp, "%s%lf%lf", cty->name, &(cty->lat), &(cty->lon) );
}
...
int main( int argc, char *argv[] )
{
  ...
  struct City *city = (struct City *) malloc( n * sizeof( struct City ) );
  struct City *c = city;
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    double shortest = -1;
    struct City *c2 = city;
    struct City *shortestCity = city;
  
    for ( int i = 0; i < n; i++ ) {
        double dist = globalDistance( lat, lon, c2->lat, c2->lon );
        if ( shortest < 0 || shortest > dist ) {
            shortestCity = c2;
            shortest = dist;
        }
        c2++;
    }
    printf( "%s %.2lf\n", shortestCity->name, shortest );
  }
